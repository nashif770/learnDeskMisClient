...
const SideBar = () => {
  ...
  return (
    <aside className="w-64 bg-teal-100 shadow-lg min-h-screen p-6 flex flex-col">
      <h2 className="text-2xl font-bold text-purple-700 mb-6 text-center">
        MIS Menu
      </h2>

      {categories
        .filter((category) => category.mvpRating <= 1) // show only categories with rating 1, 2, 3
        .map((category, idx) => (
          <div key={idx} className="mb-4">
            {/* Category Title */}
            <h3 className="text-purple-600 font-semibold text-sm px-2 py-1">
              {category.title}
            </h3>
            {/* Links */}
            <nav className="flex flex-col space-y-1 px-2">
              {category.links.map((link, index) => (
                <Link
                  key={index}
                  href={link.href}
                  className="text-gray-700 hover:text-purple-700 hover:underline transition-colors duration-200 px-2 py-1 rounded"
                >
                  {link.label}
                </Link>
              ))}
