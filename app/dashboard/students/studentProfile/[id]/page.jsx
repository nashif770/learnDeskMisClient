...
const StudentProfile = () => {
  ...
  if (!student)
    return <div className="p-6 text-center text-gray-500">No student found</div>;
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="flex flex-col items-center p-4 bg-emerald-50 rounded-lg shadow hover:shadow-lg transition">
          ...
          <p className="text-2xl font-bold text-emerald-700">{avgScore(student.monthlyTests)}</p>
        </div>
        <div className="flex flex-col items-center p-4 bg-blue-50 rounded-lg shadow hover:shadow-lg transition">
          ...
          <p className="text-2xl font-bold text-blue-700">{avgScore(student.midTerms)}</p>
        </div>
        <div className="flex flex-col items-center p-4 bg-purple-50 rounded-lg shadow hover:shadow-lg transition">
          ...
          <p className="text-2xl font-bold text-purple-700">{avgScore(student.semesters)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-4 bg-white rounded-lg shadow hover:shadow-lg transition space-y-2">
          ...
          <p><strong>Attendance %:</strong> <span className={`font-bold ${attendancePercentage >= 75 ? 'text-green-600' : attendancePercentage >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>{attendancePercentage}%</span></p>
        </div>
        <div className="p-4 bg-white rounded-lg shadow hover:shadow-lg transition space-y-2">
          <h2 className="text-xl font-semibold border-b pb-2">Marks Overview</h2>
          <p><strong>CTs Avg:</strong> {avgScore(student.monthlyTests)}</p>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <button className="flex-1 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Edit Profile</button>
        <button className="flex-1 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">View Report</button>
        <button className="flex-1 px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition">Print</button>
      </div>
    </div>
  );
};
