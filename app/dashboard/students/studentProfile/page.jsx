      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-4 bg-blue-600 px-8 py-4 rounded-[1.5rem] shadow-xl shadow-blue-100 transition-transform hover:scale-105">
            ...
            <div>
              <p className="text-[10px] font-black text-blue-200 uppercase tracking-[0.2em]">Total Records</p>
              <p className="text-2xl font-black text-white leading-none">{filteredData.length}</p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-left border-separate border-spacing-0">
              <thead>
                <tr className="bg-slate-50/50">
                  <th className="px-10 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Index</th>
                  <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Student Identity</th>
                  <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Academic Level</th>
                  <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Contact & Region</th>
                  <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Family Details</th>
                  <th className="px-10 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em] text-right">Profile</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {filteredData.length > 0 ? (
                  filteredData.map((s, index) => (
                    <tr key={s.Id} className="group hover:bg-blue-50/20 transition-all cursor-default">
                      ...
                      <td className="px-8 py-8">
                        ...
                        <p className="text-sm font-bold text-slate-500 mt-2 ml-1 italic">Roll: {s.roll || "—"}</p>
                      </td>
                      <td className="px-8 py-8">
                        <p className="text-base font-black text-slate-700">{s.mobile}</p>
                      ...
                      <td className="px-8 py-8">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm font-bold text-slate-600 truncate max-w-[150px]">{s.fatherName}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-pink-400"></span>
                            <p className="text-sm font-bold text-slate-600 truncate max-w-[150px]">{s.motherName}</p>
                          </div>
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan="6" className="py-40 text-center">
                      <div className="flex flex-col items-center justify-center space-y-4 opacity-30">
                         ...
                         <p className="text-2xl font-black text-slate-900 uppercase tracking-widest">No Matches Found</p>
                      </div>
                    </td>
                  </tr>
                )}
